...
export default function Page() {
 ...
 const bottomSheetModalRef = useRef(null);
    ...
    if (valid) {
      try {
        ...
        if (response.status === 200) {
          ...
          setLoading(false);
          setModalVisible(true);
          setModeModal("success");
          setMessageModal("Successfully change password");
        } else {
  ...
  // mendapatkan data user yang sedang logn
  async function fetchUserInfo() {
    const storedUserInfo = await AsyncStorage.getItem('userInfo');
    if (storedUserInfo) {
      const response = await authenticatedRequest(`/api/v1/public/users/${JSON.parse(storedUserInfo).id_users}`, "GET",);
      if (response.status === 200) {
  ...
  // mendapatkan data foto profil user yang sedang login
  useEffect(() => {
        ...
        <ScrollView>
          {userInfo && (
            <View style={{ paddingHorizontal: 20, paddingTop: 50, alignItems: 'center' }}>
              ...
              <View style={{ ...styles.status, backgroundColor: userInfo.status == 'VALID' ? COLORS.green : COLORS.red }}>
                <Text style={{ color: COLORS.white, fontSize: 12, fontWeight: 'bold' }}>{userInfo.status && userInfo.status}</Text>
              </View>
